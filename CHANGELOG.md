...
- **Git lineage:** Merged `upstream/main` with `--allow-unrelated-histories` to preserve both upstream commits and fork commits; ongoing work uses `origin` (e.g. `ax3cubed/product-brainstorm`) and `upstream` for the public experiment.
...
- **Firebase / deploy:** `firebase.json` and database rules updates; Firebase Storage CORS config (`firebase/storage-cors.json`, `storage.cors.json`); Vercel config (`vercel.json`).
